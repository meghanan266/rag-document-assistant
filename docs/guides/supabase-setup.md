...
3. Set a **project name** (e.g. `rag-document-assistant`).
...
- source document and chunk tables with embedding columns
- generated full-text search (`tsvector`) columns
- HNSW and GIN indexes
- chat thread, message, and citation tables
- row-level security policies

Use the **direct/session** database connection string for Alembic — not the transaction pooler URL.
